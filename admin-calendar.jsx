...
const CAL_TYPES = {
  ...
  sla_deadline:  { label:'ครบ SLA',      color:'#ea580c', bg:'#fff7ed', icon:'alert'   },
...
function CalendarPage({ officers, currentUser }) {
  ...
  const load = (y=year, m=month, oid=filterOid) => {
    ...
    api.getCalendar(y, m, oid).then(d=>{
      ...
      const slaEvs = (d.sla_deadlines||[]).map(s=>({
        ...s, _type:'sla_deadline',
        title: `SLA: ${s.subject}`,
        event_date: s.sla_deadline,
        id: 'sla_'+s.case_id,
      }));
      ...
      setData({ ...d, _merged: [...calEvs, ...dueDateEvs, ...slaEvs, ...closedEvs, ...taskEvs] });
