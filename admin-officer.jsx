...
/* ---------------- Timeline พร้อม SLA ต่อขั้นตอน ---------------- */
function StepTimeline({ events, onRefresh, canEdit }) {
  const [busy,    setBusy]    = React.useState(null);
  const [editing, setEditing] = React.useState(null); // { id, field, value }

  const SLA_COLOR = { g:'var(--ok)', a:'var(--warn)', r:'var(--danger)' };
  const SLA_LABEL = { g:'ทันกำหนด', a:'ใกล้ครบ', r:'เกินกำหนด' };

  async function markStatus(ev, st) {
    if (busy) return;
    setBusy(ev.id + ':' + st);
    try {
      await api.updateEvent(ev.id, { ev_status: st });
      await onRefresh();
    } catch(e) { alert(e.message); }
    setBusy(null);
  }

  async function saveDate(ev, field, value) {
    setBusy(ev.id + ':' + field);
    try {
      await api.updateEvent(ev.id, { [field]: value || null });
      await onRefresh();
    } catch(e) { alert(e.message); }
    setBusy(null);
    setEditing(null);
  ...
  function DateField({ ev, field, label }) {
    const val  = ev[field] || '';
    const isMe = editing?.id === ev.id && editing?.field === field;
    if (!canEdit) return <span className="sm muted">{val ? val : '—'}</span>;
    if (isMe) return (
      <span className="vcenter" style={{gap:4}}>
        <input type="date" className="input" style={{padding:'3px 7px',fontSize:12,width:140}}
          defaultValue={val || new Date().toISOString().slice(0,10)}
          onKeyDown={e=>{ if(e.key==='Enter') saveDate(ev, field, e.target.value); if(e.key==='Escape') setEditing(null); }}
          autoFocus
          id={`df-${ev.id}-${field}`}/>
        <button className="btn btn-sm btn-primary" style={{padding:'3px 8px',fontSize:11}}
          onClick={()=>{
            const el = document.getElementById(`df-${ev.id}-${field}`);
            if(el) saveDate(ev, field, el.value);
          }}>บันทึก</button>
        <button className="btn btn-sm btn-ghost" style={{padding:'3px 6px',fontSize:11}}
          onClick={()=>setEditing(null)}>ยกเลิก</button>
      </span>
    );
    return (
      <button className="btn btn-ghost btn-sm" style={{fontSize:11,padding:'2px 7px',gap:4}}
        onClick={()=>setEditing({ id:ev.id, field, value:val })}>
        <Icon name="calendar" style={{width:11,height:11}}/>
        {val || label}
      </button>
    );
  }
  ...
  return (
    <div className="timeline">
      {events.map((e, i) => {
        const hasStep  = !!e.step_key;
        const isDone   = e.st === 'done';
        const isActive = e.st === 'active';
        const color    = hasStep && e.step_sla ? SLA_COLOR[e.step_sla] : undefined;

        return (
          <div key={e.id || i} className="tl-item">
            <div className={"tl-dot " + (isDone ? "done" : isActive ? "active" : "")}>
              <Icon name={e.ic} style={{width:14,height:14}}/>
            </div>
            <div className="tl-body" style={{flex:1}}>
              {/* หัวข้อ + badge สถานะ step */}
              <div className="between" style={{alignItems:'flex-start',gap:8}}>
                <div className="tt">{e.t}</div>
                {hasStep && e.step_sla && (
                  <span style={{fontSize:11,fontWeight:600,color,background:`color-mix(in srgb,${color} 12%,transparent)`,
                    padding:'2px 8px',borderRadius:12,whiteSpace:'nowrap',flexShrink:0}}>
                    {SLA_LABEL[e.step_sla]}
                  </span>
                )}
              </div>
              ...
              {hasStep && (
                <div style={{marginTop:8,padding:'8px 10px',background:'var(--surface-2)',
                  ...
                  <div style={{display:'flex',gap:16,flexWrap:'wrap',marginBottom:6}}>
                    <div>
                      <div style={{fontSize:11,color:'var(--ink-3)',marginBottom:2}}>วันที่เริ่ม</div>
                      <DateField ev={e} field="started_at" label="บันทึกวัน"/>
                    </div>
                    <div>
                      <div style={{fontSize:11,color:'var(--ink-3)',marginBottom:2}}>วันที่เสร็จ</div>
                      <DateField ev={e} field="completed_at" label="บันทึกวัน"/>
                    </div>
                    <div style={{marginLeft:'auto',textAlign:'right'}}>
                      <div style={{fontSize:11,color:'var(--ink-3)',marginBottom:2}}>กรอบเวลา</div>
                      <span className="tnum sm" style={{fontWeight:600}}>{e.step_days_allowed} วัน</span>
                    </div>
                  </div>

                  {/* progress bar */}
                  {e.step_days_used !== null && (
                    <>
                      <div style={{height:6,borderRadius:3,background:'var(--surface-3,var(--border))',overflow:'hidden',marginBottom:4}}>
                        <div style={{
                          height:'100%',
                          width: Math.min(100, Math.round(e.step_days_used / e.step_days_allowed * 100)) + '%',
                      ...
                      <div className="between" style={{fontSize:11}}>
                        <span style={{color:'var(--ink-3)'}}>ใช้ไป {e.step_days_used} วัน</span>
                        <span style={{color, fontWeight:600}}>
                          {e.step_days_remain >= 0
                            ? `เหลือ ${e.step_days_remain} วัน`
                            : `เกิน ${Math.abs(e.step_days_remain)} วัน`}
                        </span>
                      </div>
                    </>
                  )}
                  ...
                  {/* ปุ่มเปลี่ยนสถานะ */}
                  {canEdit && (
                    <div className="vcenter" style={{gap:6,marginTop:8}}>
                      {!isDone && !isActive && (
                        <button className="btn btn-sm" style={{fontSize:11,padding:'3px 10px',background:'var(--info)',color:'#fff'}}
                          disabled={!!busy} onClick={()=>markStatus(e,'active')}>
                          {busy===e.id+':active' ? '…' : '▶ เริ่มดำเนินการ'}
                        </button>
                      )}
                      {isActive && (
                        <button className="btn btn-sm btn-primary" style={{fontSize:11,padding:'3px 10px'}}
                          disabled={!!busy} onClick={()=>markStatus(e,'done')}>
                          {busy===e.id+':done' ? '…' : '✓ บันทึกว่าเสร็จแล้ว'}
                        </button>
                      )}
                      {isDone && (
                        <button className="btn btn-sm btn-ghost" style={{fontSize:11,padding:'3px 10px'}}
                          disabled={!!busy} onClick={()=>markStatus(e,'active')}>
                          ↩ ย้อนกลับ
                        </button>
                      )}
      ...
      <div className="grid" style={{gridTemplateColumns:"1.7fr 1fr",alignItems:"start"}}>
        <div className="card">
          ...
          <div className="card-pad" style={{padding:24}}>
            ...
            {tab==="timeline" && <StepTimeline events={c.events} onRefresh={()=>{
