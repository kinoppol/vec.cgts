...
function CaseTimeline({ steps = [], onRefresh, canEdit }) {
  ...
  const [doneModal, setDoneModal] = React.useState(null); // step object
            ...
            <div style={{flex:1,paddingLeft:12,paddingBottom: isLast ? 0 : 16,paddingTop:1}}>
              ...
              {(s.detail || s.attachment_name) && (
                <div style={{marginTop:6,display:'flex',flexDirection:'column',gap:4}}>
                  ...
                  {s.attachment_name && (
                    <div className="vcenter" style={{gap:6,fontSize:12,color:'var(--info)'}}>
                      <Icon name="file" style={{width:13,height:13,color:'var(--danger)'}}/>
                      <span>{s.attachment_name}</span>
                      {s.attachment_size && <span style={{color:'var(--ink-3)'}}>({s.attachment_size})</span>}
                    </div>
                  )}
                </div>
              )}
