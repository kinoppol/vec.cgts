...
if ($confirm === 'lawyer_dispatch') {
    ...
    echo '<h2>Migration [27]: ข้อสั่งการ + ส่งเรื่องให้นิติกร</h2><pre>';
    try {
        $db = getDB();
        $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $cols = $db->query("SHOW COLUMNS FROM cases")->fetchAll(PDO::FETCH_COLUMN);
        foreach ([
            'lawyer_note    TEXT     DEFAULT NULL AFTER lawyer_id',
            'lawyer_note_by INT      DEFAULT NULL AFTER lawyer_note',
            'lawyer_note_at DATETIME DEFAULT NULL AFTER lawyer_note_by',
            'lawyer_sent_at DATETIME DEFAULT NULL AFTER lawyer_note_at',
        ] as $col) {
            $name = explode(' ', $col)[0];
            if (!in_array($name, $cols)) {
                ...
                echo "– cases.$name มีอยู่แล้ว ข้าม\n";
            }
        }
...
if ($confirm === 'lawyer_id') {
    ...
    try {
        ...
        $cols = $db->query("SHOW COLUMNS FROM cases")->fetchAll(PDO::FETCH_COLUMN);
        if (!in_array('lawyer_id', $cols)) {
            $db->exec("ALTER TABLE cases ADD COLUMN lawyer_id VARCHAR(10) DEFAULT NULL AFTER assigned_group");
            echo "✓ ALTER cases ADD lawyer_id\n";
            // เพิ่ม FK (ไม่ critical ถ้าพลาด)
            try {
                $db->exec("ALTER TABLE cases ADD CONSTRAINT fk_case_lawyer FOREIGN KEY (lawyer_id) REFERENCES officers (id) ON DELETE SET NULL");
                echo "✓ ADD FK fk_case_lawyer\n";
            } catch (Throwable $e) { echo "– ข้าม FK: ".$e->getMessage()."\n"; }
        } else { echo "– cases.lawyer_id มีอยู่แล้ว ข้าม\n"; }
...
if ($confirm === 'event_datetime') {
    ...
    try {
        ...
        $cols = $db->query("SHOW COLUMNS FROM case_events")->fetchAll(PDO::FETCH_ASSOC);
        $types = array_column($cols, 'Type', 'Field');

        if (stripos($types['started_at'] ?? '', 'datetime') === false) {
            $db->exec("ALTER TABLE case_events MODIFY started_at DATETIME DEFAULT NULL");
            echo "✓ ALTER case_events.started_at DATE → DATETIME\n";
        } else {
            echo "– case_events.started_at เป็น DATETIME แล้ว ข้าม\n";
        }
        if (stripos($types['completed_at'] ?? '', 'datetime') === false) {
            $db->exec("ALTER TABLE case_events MODIFY completed_at DATETIME DEFAULT NULL");
            echo "✓ ALTER case_events.completed_at DATE → DATETIME\n";
        } else {
            echo "– case_events.completed_at เป็น DATETIME แล้ว ข้าม\n";
        }
        echo '<span class="ok">✓ เสร็จสิ้น</span>';
    } catch (Throwable $e) {
        echo '<span class="err">✗ '.$e->getMessage().'</span>';
